...
const Gallery = ({ productImage }: { productImage: string[] }) => {
  if (!productImage.length) return null;
  ...
  // Auto switch main image
  useEffect(() => {
    if (isHovered) return;
    autoSwitchRef.current = setInterval(() => {
      setMainImage((prev) => {
        const currentIndex = productImage.indexOf(prev);
  ...
  return (
    ...
    >
      {/* Thumbnails */}
      <div className="w-full lg:w-32 flex lg:flex-col overflow-x-auto lg:overflow-y-auto scrollbar-hide relative">
